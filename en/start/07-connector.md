...
`--http-file` — JSON responses for offline `net.http` (key = URL without query). Need Helix/API without network — see [CLI fixtures](https://github.com/CaptainGnome/modus-sdk/tree/main/cli/tests/fixtures) and the dummy [`modus-examples/connector-replay`](https://github.com/CaptainGnome/modus-examples/tree/master/connector-replay).
