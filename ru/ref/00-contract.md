...
CLI из [modus-sdk](https://github.com/CaptainGnome/modus-sdk) (не crates.io). Из корня clone SDK:

```powershell
cargo run --manifest-path cli/Cargo.toml --release -- <команда>
```

- `--manifest-path cli/Cargo.toml` — пакет CLI; вы в корне `modus-sdk`.
- В корне продукта (submodule): `modus-sdk/cli/Cargo.toml`.
